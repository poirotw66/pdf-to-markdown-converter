...
def _convert_pdf_page_to_image(pdf_path: str, page_num: int, dpi: int = 150) -> Image.Image:
    ...
    try:
        ...
        raise ValueError(f"Failed to convert page {page_num} to image: {str(e)}") from e
...
class PDFParser:
    ...
    def parse_pdf(
        ...
        Args:
            ...
            prompt_template: Optional prompt id or text override
        ...
        for page_num in pages_to_process:
            ...
            needs_gemini = not self.force_pymupdf and any([
                density < self.text_density_threshold,
                has_table,
                len(text.strip()) < 50,  # Very short text might be image-based
            ])
